...
class CrossEntropyCost(object):
    ...
    def derivative(a, y):
        ...
        if a.all == 0 or a.all == 1: # to avoid ambiguity, use a.any() or a.all()
            return 0
        else:
            print('QA : %s' %str((a - y) / (a * (1-a))))
...
class Network(object):
    ...
    def set_parameters(self, cost=QuadraticCost, act_hidden=Sigmoid,
        ...
        # if cost function was set to anything besides QuadracticCost when act_output is equal to tanh set it to QuadraticCost
        
        if self.act_output == Tanh and self.cost != QuadraticCost:
            # printing warning
            print("Tanh only accepts Sigmoid cost function. Changing to QuadracticCost.")
            self.cost = QuadraticCost
        
        # LogLikelihood cost fucntion should only be used when act_output is equal for Softmax
        if (self.act_output == Softmax and self.cost != LogLikelihood):
            # print warning 
            print('Softmax only accepts LogLikelihood cost function so changing to LogLikelihood')
            self.cost = LogLikelihood
            
        
        if self.act_hidden == Softmax:
            print('changing the hidden activation to Sigmoid')
    ...
    def backprop(self, x, y):
        ...
        for b, w in zip(self.biases, self.weights):
            ...
            #        activation *= self.dropoutmask[actL_count - 1]
            #        activations.append(activation)
            #actL_count += 1
        ...
        for l in range(2, self.num_layers):
            ...
            print('length of activations %s when l is %s'%(str(len(activations)), str(l)))
            print('activations[-2] :%s' %str(activations[-2]))
            #print('activations[-3] :%s' %str(activations[-3]))
            #print('activations[-l-1] : %s' %str(activations[-l-1]))
            # print('test: %s' %str(np.dot(delta, activations[-l-1].transpose())))
